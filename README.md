...
- **`@types/node` lives in `dependencies`, not `devDependencies`.** Obsidian's community-review type-checker installs only `dependencies` before running `tsc`/eslint, so a `@types/node` left in `devDependencies` (the conventional place for it) silently disappears from that pass — `fs` and `child_process` imports then type as `any`, cascading into a wall of `@typescript-eslint/no-unsafe-*` findings in `src/runner.ts` that a normal `npm install` never reproduces locally. Verified with `npm install --omit=dev` in a clean checkout: `@types/node` is only present in `node_modules/@types` once it's listed under `dependencies`.
